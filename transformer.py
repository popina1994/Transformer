...
class Transformer:
    ...
    def forward_pass(self, X_in: torch.Tensor, X_out: torch.Tensor)->torch.Tensor:
        encoder_out = self.encoder.forward_pass(X_in=X_in)
        print(f"Encoder out{encoder_out=}")
        decoder_out = self.decoder.forward_pass(X_in=X_out,
                                           encoder_K=self.encoder.get_K(),
                                           encoder_V=self.encoder.get_V())
